...
// hier folgen Optimierungen (FB 20120507)
...
//*******************************************************************************************************************************
// Nach Optimierung: nur noch Sektor/Cluster-Verwaltung
void _after_ffwrite(void)
{
...
void ffwrite(char c)
{
	fat.sector[file.cntOfBytes++] = c;							// schreiben des chars auf den puffer sector und zaehler erhoehen (post-increment)
    _after_ffwrite();
}
#endif
...
#include <string.h> // gehoert eigentlich in den Kopf der Datei
...
// TODO: test speed with benchmark
void ffwriten(uint8_t *s, uint16_t n)
{
    for (;;)
    {
        uint16_t bytes = 512 - file.cntOfBytes; // number of free bytes in buffer
        
        if (n < bytes)
            bytes = n;
        
        // copy source data into sector buffer
        memcpy(&fat.sector[file.cntOfBytes], s, bytes);
        file.cntOfBytes += bytes;
        _after_ffwrite();
       
        n -= bytes; // reduce number of bytes to write
        if (n == 0)
            break;
        s += bytes; // advance data pointer
    }
    fat.bufferDirty = 1;
}
